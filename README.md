...
The first hyperparameter we attempted to adjust was the temperature value. In our model, temperature is used to control the randomness of the predicted notes by scaling the logits before applying our activation function. By increasing the temperature, you increase the randomness/entropy, leading to more surprising notes being generated and vice versa. Originally with our base temperature of 2, the songs lacked originality and felt generic because each new song we generated was primarily composed of same the pitch, step, and duration as the mean of our original training dataset. Depending on the model, this can be a good thing. However, for our music generation model, the base temperature resulted in subjectively less pleasing or boring music due to being less random. As we progressively increase the temperature, all of the pitch, steps, and durations of notes generated become closer to a uniform distribution, which for our model is exactly what we needed. By increasing the entropy, or increasing the probability of a note with a pitch, step, or duration to be different from the training mean, our songs became more creative. 
...
| ---- | ----- | ----- | ----- | ------- | ----- |
| Task completed | research, experimentation with changing training set, experimentation with changing generating song, updated report | Create github pages, problem definition, references, powerpoint, improved introduction, made histogram visualizations, tuned temperature hyperparameter, helped generate songs with different instruments | Added MusPy Source, Filmed and edited proposal video, conducted more research, updated report, helped debug and code python notebook, created final presentation, filmed and edited final presentation | Found Briot source, added information to introduction, Potential Results, Part of powerpoint, made piano note visualizations of midi data, helped debug and code python notebook. Tested with tuning learning rate, added logging and automatic data saving to notebook | Dataset cleaning, setup model architecture, results & discussion section, added additional dense layer |
